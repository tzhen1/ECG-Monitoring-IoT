...
#define AFE4400_SPI_SPEED 2000000
SPISettings SPI_SETTINGS(AFE4400_SPI_SPEED, MSBFIRST, SPI_MODE0); 
...
{
  afe44xxWrite(CONTROL0, 0x000001); // 1 = enables SPI read
  IRtemp = afe44xxRead(LED1VAL); // ir value
  afe44xxWrite(CONTROL0, 0x000001);
  REDtemp = afe44xxRead(LED2VAL);
  afe44xx_data_ready = true;

  IRtemp = (unsigned long) (IRtemp << 10); // readout shifted by 10, give space for next data
  afe44xx_raw_data->IR_data = (signed long) (IRtemp); // put into IR_data in structure afe.h
...
void AFE44XX :: afe44xxWrite (uint8_t address, uint32_t data) // address = register
{
  SPI.beginTransaction(SPI_SETTINGS);
  digitalWrite (_cs_pin, LOW); // enable device
  SPI.transfer (address); // send address to device
  SPI.transfer ((data >> 16) & 0xFF); // write top 8 bits, shift to 8th position as send (8:0)  (0xFF , 1 mask for select desired)
